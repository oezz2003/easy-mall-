...
export default function WomenShopPage() {
  const products = useMemo(() => {
    return allProducts.filter(p => p.tags.includes('women'));
  }, []);
    ...
    <>
      <section className="relative h-[40vh] w-full flex items-center justify-center bg-background">
        ...
        <div className="relative z-10 text-center text-white">
          <h1 className="font-headline text-5xl md:text-6xl font-bold">Women's Collection</h1>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 md:px-6 py-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Discover the latest in women's fashion.</p>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="group relative rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 min-h-[400px]">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center h-full text-center text-white p-8">
              <h3 className="font-headline text-3xl font-bold">For Him</h3>
              <p className="mt-2 text-white/90">Explore our men's collection</p>
              <Button asChild className="mt-4 bg-accent text-accent-foreground hover:bg-accent/90">
                <Link href="/shop/men">Shop Now <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </div>
          </div>
          <div className="group relative rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 min-h-[400px]">
